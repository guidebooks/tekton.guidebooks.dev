...
export default async function graph2doms(
  ...
  const svg = container.append("g").attr("id", "wskflowMainG").attr("class", "kui--screenshotable")
        ...
        if (activations) {
          ...
          if (d.children) {
            ...
            if (d.multiLineLabel) {
              ...
              qtipText = `<span class='qtip-prefix ${d.type}' style="padding-right:5px; ">${typeForDisplay} |</span> ${
                d.label || d.tooltip || d.prettyCode
              }`
            }
            ...
            qtipText = `<div class='qtip-prefix let' style="margin-bottom:1ex; padding-right:5px; ">${d.type}</div>${
              d.label || d.tooltip
            }`
            ...
            if (d.type === "Exit") {
              ...
              if (d.visited.length === 1) {
                ...
                d.visited.forEach((n) => {
                  ...
                  lis += `<span class='actItem' style='color:${c}; text-decoration:underline; cursor: pointer;' index=${n}>${timeString}</span> (${
                    duration + unit
                  })<break></break>`
      ...
      .text(function (d) {
        if (d.type === "retain" || (d.type === "Dummy" && d.label === d.id)) {
          ...
          if (typeof d.value === "string") {
            ...
            const keys = Object.keys(d.value).slice(0, maxLen).concat(rest)
            return `{${keys}}`
          }
